...
const APP_MAP = [
  ...
  '- AI ANSWERING tab: their phone line. Get a number, switch answering on/off, pick the agent name and voice (10 voices, Spanish included), custom greeting, transfer number, ON-CALL LIST for emergencies (days: transfer number first; nights 8pm-8am: on-call list first), business hours, booking slot length, timezone, FAQ text the AI may quote, call recording toggle, SPAM FILTER toggle (on by default: hangs up on robocalls, screens no-caller-ID calls with one question, auto-blocks repeat offenders after 3 strikes; filtered and screened calls never use plan minutes; known callers are never screened; each call in Recent calls has a "block this number" link), voicemail message for call-backs, recent calls with recordings and transcripts, and the monthly stats.',
...
function stateBrief(account, owner) {
  ...
  // Voice config lives per business now — brief from the business that owns
  // a line, else the first one.
  const profs = db.getProfiles(account.id);
  const prof = profs.find((p) => p.voice && p.voice.number) || profs[0] || {};
  const v = db.profileVoice(account, prof.id ? prof : null);
  const bits = [
