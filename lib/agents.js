  ...
  while (totalAdded < count && wavesRun < maxWaves && emptyWaves < 2) {
    ...
    if (emailApi.enabled(account)) {
      for (let i = 0; i < fresh.length; i++) {
        const r = fresh[i];
        try {
          if (r.email_confidence === 'pattern' || r.email_confidence === 'invalid' || !r.email) {
            // guessed or missing → look up the real one
            const hit = await emailApi.findEmail(account, { name: r.name, website: r.website, email: r.email });
            if (hit && hit.confidence) {
              fresh[i] = { ...r, email: hit.email, email_confidence: hit.confidence, email_source: hit.source };
              if (hit.confidence === 'verified') verified++;
            }
          } else if (r.email) {
            // site/role email → verify it actually exists
            const v = await emailApi.verifyEmail(account, r.email);
            if (v) { fresh[i] = { ...r, email_confidence: v.confidence, email_source: v.source };
              if (v.confidence === 'verified') verified++; }
          }
        } catch (e) {
          db.logActivity(accountId, { agent: 'PROSPECTOR', profileId, msg: `Email lookup: ${e.message}` });
