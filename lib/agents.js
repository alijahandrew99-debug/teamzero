...
function footerFor(p) { return `${p.name || 'Dawnpipe'} · ${FOOTER}`; }
...
async function prospect(accountId, profileId, { count = 5, hints = '', onProgress } = {}) {
  ...
  while (totalAdded < count && wavesRun < maxWaves && emptyWaves < 2) {
    ...
    emptyWaves = 0; // progress made — keep hunting
    fresh = fresh.slice(0, count - totalAdded); // never overshoot the ask
      ...
      for (let i = 0; i < fresh.length; i++) {
        ...
        try {
          ...
          if (named && (r.email_confidence === 'pattern' || r.email_confidence === 'invalid' || !r.email)) {
            ...
            if (hit && hit.confidence) { fresh[i] = { ...r, email: hit.email, email_confidence: hit.confidence, email_source: hit.source }; hHit++; if (hit.confidence === 'verified') verified++; }
            else hMiss++;
          } else if (!named) {
            ...
            if (ds) {
              ...
              hFell++; if (ds.confidence === 'verified') verified++;
            } else hMiss++;
          } else if (r.email) {
            // real person with a site/role email → verify it actually exists
            const v = await emailApi.verifyEmail(account, r.email);
            if (v) { fresh[i] = { ...r, email_confidence: v.confidence, email_source: v.source }; if (v.confidence === 'verified') verified++; }
          }
        } catch (e) {
          db.logActivity(accountId, { agent: 'PROSPECTOR', profileId, msg: `Email lookup stopped: ${e.message}` });
          break; // bad key — stop burning calls this run
        }
      }
      db.logActivity(accountId, { agent: 'PROSPECTOR', profileId, msg: `Hunter: ${hHit} found, ${hFell} recovered by domain, ${hMiss} no match` });
    }
  ...
  const short = totalAdded < count ? ` — this niche/focus ran dry at ${totalAdded} of ${count}; try a broader focus or different region` : '';
  db.logActivity(accountId, { agent: 'PROSPECTOR', profileId, msg: `Done — ${totalAdded} real lead(s) found${vnote}${short}` });
...
module.exports = { prospect, outboundRun, scoutBrief, autofillFromWebsite };
