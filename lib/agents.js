...
async function autofillFromWebsite(url) {
  ...
  const prompt = [
    `You are onboarding a business into Dawnpipe. Dawnpipe answers their phone with an AI receptionist and finds them new customers by email. From the website content below, draft BOTH their sales profile and the script their AI phone agent will follow.`,
    '',
  ...
  const raw = await callAI(prompt, { maxTokens: 3000, tier: 'draft' });
