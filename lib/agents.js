...
async function fetchPage(url, ms = 8000) {
  try {
    const r = await fetch(url, { signal: AbortSignal.timeout(ms), redirect: 'follow',
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; TeamZero/1.0)' } });
    if (!r.ok) return '';
...
async function autofillFromWebsite(url) {
  ...
  const prompt = [
    `You are onboarding a business into TeamZero (an AI sales tool). From the website content below, draft their sales profile so their AI sales team knows who to sell for.`,
    '',
