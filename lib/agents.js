...
function stampFooter(body, profile, unsubUrl) {
  ...
  const lines = ['—', `${(profile && profile.name) || ''}${addr ? ' · ' + addr : ''}`.trim(),
    unsubUrl ? `Unsubscribe: ${unsubUrl}` : ''].filter(Boolean);
  return txt + '\n\n' + lines.join('\n');
}
...
async function fetchPage(url, ms = 8000) {
  try {
    const r = await fetch(url, { signal: AbortSignal.timeout(ms), redirect: 'follow',
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; Dawnpipe/1.0)' } });
    if (!r.ok) return '';
...
async function autofillFromWebsite(url) {
  let u = (url || '').trim();
  if (!u) throw new Error('Enter your website URL');
  if (!/^https?:\/\//i.test(u)) u = 'https://' + u;
  let base; try { base = new URL(u); } catch { throw new Error('That doesn\'t look like a valid URL'); }
  const origin = base.origin;
  const pages = await Promise.all([u, `${origin}/about`, `${origin}/about-us`, `${origin}/services`]
    .map((p) => fetchPage(p)));
  const text = pages.filter(Boolean).join('\n\n').slice(0, 14000);
  if (!text) throw new Error('Couldn\'t read that site — check the URL, or fill the fields by hand.');

  const prompt = [
    `You are onboarding a business into Dawnpipe (an AI sales tool). From the website content below, draft their sales profile so their AI sales team knows who to sell for.`,
    '',
    `WEBSITE (${base.hostname}):`,
    text,
    '',
    `Return STRICT JSON only, no markdown:`,
    `{"name":"","senderName":"","offer":"","icp":"","valueProp":"","proof":"","tone":"","cta":""}`,
  ...
  const raw = await callAI(prompt, { maxTokens: 1500, tier: 'draft' });
  const j = safeJSON(raw) || {};
  return {
    name: j.name || base.hostname.replace(/^www\./, ''),
    senderName: j.senderName || '',
    offer: j.offer || '', icp: j.icp || '', valueProp: j.valueProp || '',
    proof: j.proof || '', tone: j.tone || 'Direct, warm, concrete. No fluff.',
    cta: j.cta || 'A short reply, or a 15-minute call.',
  };
}
  ...
  // BEFORE spending AI money, not silently discovered lead-by-lead.
  {
    ...
    if (emailApi.enabled(acct)) {
      ...
      if (st && st.searches && st.searches.available > 0 && st.searches.used >= st.searches.available) {
        hunterDown = true;
        db.logActivity(accountId, { agent: 'PROSPECTOR', profileId, msg: `⚠ Hunter quota used up (${st.searches.used}/${st.searches.available} searches on the ${st.plan} plan, resets ${st.resetDate}). Only site-published emails will be kept this run.` });
      }
    } else {
      db.logActivity(accountId, { agent: 'PROSPECTOR', profileId, msg: `⚠ No email-data key connected — only site-published emails will be kept. Connect Hunter.io in Your Business tab for verified personal emails.` });
    }
  }
    ...
    if (emailApi.enabled(account) && !hunterDown) {
      ...
      for (let i = 0; i < fresh.length; i++) {
        ...
        try {
          ...
          // Quota exhausted / bad key: say so LOUDLY (this is the #1 cause of
          // "why is it still guessing") and stop burning calls this run.
          hunterDown = true;
          db.logActivity(accountId, { agent: 'PROSPECTOR', profileId, msg: `⚠ ${e.message}` });
          break;
        }
      }
      if (!hunterDown) db.logActivity(accountId, { agent: 'PROSPECTOR', profileId, msg: `Hunter: ${hHit} found, ${hFell} recovered by domain, ${hMiss} no match` });
    }
...
async function outboundDraft(p, lead) {
  const prompt = [
    `You are OUTBOUND, the email SDR on Dawnpipe. Write ONE cold email for the client below to this lead.`,
    '', profileBlock(p),
    ...
    `Return STRICT JSON only: {"subject":"...","body":"..."} — body ends with a simple sign-off from ${p.senderName || 'the sender'}. Do NOT add a footer, disclaimer, address or unsubscribe line: those are stamped on automatically at send time, and writing your own produces two of them.`,
  ].join('\n');
...
async function outboundRun(accountId, profileId, onProgress) {
  ...
  const acct = db.getAccount(accountId);
  const owner = require('./stripe').isOwner(acct);
  for (const lead of leads) {
    ...
    try {
      ...
      const spam = spamcheck.analyze({ subject: d.subject, body: d.body });
      ...
      if (!owner) plans.consume(acct, 1, false);
      ok++;
    } catch (e) {
...
module.exports = { stampFooter, footerFor, prospect, outboundRun, scoutBrief, autofillFromWebsite };
