...
// Paid once, on an account's FIRST collected invoice. This is the lever that
// keeps a commission-only rep alive through ramp — the gap between closing and
// the first recurring payment is what kills them at 30-60 days.
const ACTIVATION_BONUS_CENTS = Number(process.env.REP_ACTIVATION_BONUS_CENTS || 15000);
...
// ---- referral codes ----
// Human-shaped: a rep reads this down a phone and types it into a browser, so
// no characters that look like each other (0/O, 1/I/l) and no vowels, which
// keeps accidental words out.
const CODE_ALPHABET = 'ACDEFGHJKMNPQRTUVWXY34679';
function makeCode(seed = '') {
  const base = String(seed).toUpperCase().replace(/[^A-Z]/g, '').slice(0, 3);
  let tail = '';
  const bytes = crypto.randomBytes(5);
  for (let i = 0; i < 5; i++) tail += CODE_ALPHABET[bytes[i] % CODE_ALPHABET.length];
  return ((base || 'REP') + tail).slice(0, 8);
}
...
function createRep({ name, email, phone = '', notes = '' }) {
  ...
  // Vanishingly unlikely, but a duplicate code would silently misattribute a
  // paycheck, so it is worth the loop.
  let guard = 0;
  while (repByCode(rep.code) && guard++ < 20) rep.code = makeCode(name);
...
function recordCollection({ account, invoiceId, amountCents, periodStart = '', periodEnd = '', description = '' }) {
  ...
  if (monthIndex === 1 && !alreadyPosted('activation', account.id)) {
    rows.push(post({
...
module.exports = {
  makeCode, normaliseCode, listReps, createRep, updateRep, repByCode, repById, repByEmail, payoutBlocked,
