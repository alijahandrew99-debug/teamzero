...
// A rep may carry their OWN intro rate (rep.rateBps) -- a founder's deal for
// an early closer, set by the owner on the rep card. The 24-month window and
// the 10% tail stay global; only the intro percentage varies.
function rateBpsFor(monthIndex, rep) {
  const intro = rep && Number(rep.rateBps) >= 100 ? Number(rep.rateBps) : RATE_INTRO_BPS;
  return monthIndex <= INTRO_MONTHS ? intro : RATE_TAIL_BPS;
}
...
/**
 * Delete a rep. Only a rep with NO history can be deleted outright: once an
 * account is attributed to them or a ledger row exists, the record is part of
 * somebody's audit trail (theirs, or a customer's) and deleting it would
 * orphan money. Those reps are deactivated instead, which stops new
 * attributions and the maintenance tail while keeping every row traceable.
 */
function deleteRep(id) {
  const rep = repById(id);
  if (!rep) throw new Error('No such rep.');
  const accounts = db.allAccounts().filter((a) => a.repId === id).length;
  const rows = ledgerFor(id).length;
  if (accounts || rows) {
    const why = [accounts ? `${accounts} customer${accounts === 1 ? '' : 's'} attributed` : '', rows ? `${rows} ledger row${rows === 1 ? '' : 's'}` : ''].filter(Boolean).join(' and ');
    throw new Error(`${rep.name} has ${why}, so the record has to stay for the audit trail. Deactivate them instead — no new customers land on their code and they earn nothing further.`);
  }
  const all = db.readCollection('reps').filter((r) => r.id !== id);
  // db has no deleteCollection helper; rewrite the file through the same path.
  const fs = require('fs');
  fs.writeFileSync(require('path').join(db.DATA, 'reps.json'), JSON.stringify(all, null, 2));
  return rep;
}
...
/**
 * The OWNER assigns a customer to a rep by hand — the rep closed them on the
 * phone and the customer never clicked the link or typed the code. This is
 * the safety net for the paycheck, so it is allowed, logged with a reason, and
 * (with force) can even move an attributed account — but a move never
 * touches ledger rows already posted, and the previous rep is recorded.
 */
function assign(account, repId, { by = 'owner', reason = '', force = false } = {}) {
  if (!account) return { ok: false, reason: 'No account.' };
  const rep = repById(repId);
  if (!rep) return { ok: false, reason: 'No such rep.' };
  if (rep.status === 'terminated') return { ok: false, reason: 'That rep was terminated for cause and cannot be assigned customers.' };
  if (account.repId && account.repId !== rep.id && !force) {
    const cur = repById(account.repId);
    return { ok: false, reason: `This customer is already attributed to ${cur ? cur.name : 'another rep'}. Moving them takes money from one rep to another — tick "move it anyway" and give a reason.` };
  }
  if (account.repId === rep.id) return { ok: true, rep, unchanged: true };
  const prev = account.repId ? repById(account.repId) : null;
  db.updateAccount(account.id, {
    repId: rep.id, repCode: rep.code, repAttributedAt: account.repAttributedAt || db.nowISO(), repSource: 'owner-assigned',
    repAssignedBy: by, repAssignedAt: db.nowISO(), repAssignReason: String(reason || '').slice(0, 300),
    ...(prev ? { repPrevious: prev.id } : {}),
  });
  db.logActivity(account.id, { agent: 'SYSTEM', msg: `Assigned to rep ${rep.name} (${rep.code}) by ${by}${prev ? ` — moved from ${prev.name}` : ''}${reason ? ` — ${reason}` : ''}` });
  return { ok: true, rep, movedFrom: prev || null };
}
...
function recordCollection({ account, invoiceId, amountCents, periodStart = '', periodEnd = '', description = '' }) {
  ...
  const rateBps = rateBpsFor(monthIndex, rep);
...
module.exports = {
  makeCode, lastNameCode, normaliseCode, listReps, createRep, updateRep, setCode, deleteRep, repByCode, repById, repByEmail, payoutBlocked,
  attribute, assign, recordCollection, recordReversal, releaseHolds, summary,
