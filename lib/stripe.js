...
/** Credit a paid top-up, idempotent on the Checkout Session id. */
function creditTopup(account, kind, sessionId) {
  const t = plans.TOPUPS[kind];
  if (!t || !account) return false;
  const seen = Array.isArray(account.topupSessions) ? account.topupSessions : [];
  if (sessionId && seen.includes(sessionId)) return false;
  db.updateAccount(account.id, {
    topupLeads: Math.max(0, Number(account.topupLeads || 0)) + t.leads,
    topupMinutes: Math.max(0, Number(account.topupMinutes || 0)) + t.minutes,
    topupSessions: [...seen, sessionId].slice(-50),
  });
  db.logActivity(account.id, { agent: 'SYSTEM', msg: `Top-up credited: ${t.name} ($${t.price})` });
  return true;
}
...
function applyEvent(event) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (acc) db.trackFunnel(acc.id, 'upgrade');
