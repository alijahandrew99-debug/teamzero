...
async function reconcileCheckout(account, sessionId) {
  ...
  // Only trust a session that is actually paid AND belongs to this account —
  // otherwise a guessed/borrowed session id would unlock someone else's app.
  if (s.payment_status !== 'paid' && s.status !== 'complete') return false;
  if (s.client_reference_id && s.client_reference_id !== account.id) return false;
  const tier = (s.metadata && s.metadata.tier) || account.tier || 'starter';
  db.updateAccount(account.id, {
    stripeCustomerId: s.customer || account.stripeCustomerId,
    stripeSubscriptionId: s.subscription || account.stripeSubscriptionId || null,
    subStatus: 'active', plan: 'pro', tier, dunningSince: '',
  });
