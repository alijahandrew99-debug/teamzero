 ...
 */
// "Software as a service (SaaS) — business use". Setting it clears the
// dashboard's "Needs info" badge and makes Stripe Tax correct if it is ever
// switched on. Checkout never needed it; the badge just looked broken.
const TAX_CODE_SAAS = process.env.STRIPE_TAX_CODE || 'txcd_10103001';

/** Backfill the tax category on any Dawnpipe product that lacks one. */
async function ensureTaxCodes() {
  const ids = plans.productsMissingTaxCode();
  const done = [];
  for (const id of ids) {
    try {
      await stripePost(`products/${id}`, { tax_code: TAX_CODE_SAAS });
      plans.clearMissingTaxCode(id);
      done.push(id);
    } catch (e) { console.error(`could not set tax code on ${id}:`, e.message); }
  }
  return done;
}
...
async function createMissingPrices() {
  ...
  for (const t of plans.tierList()) {
    ...
    const price = await stripePost('prices', {
      ...
      metadata: { tier: t.id },
      product_data: { name: `Dawnpipe ${t.name}`, metadata: { tier: t.id }, tax_code: TAX_CODE_SAAS },
    });
 ...
 */
/**
 * Cancel at period end, straight through the Stripe API. This is the cancel
 * button's backend, and it deliberately does NOT depend on the hosted billing
 * portal: the portal 500s if its live-mode configuration was never saved in
 * the dashboard, and "the cancel button is broken" is how a $499 subscription
 * becomes a chargeback and a scam claim. Cancelling at period end (not
 * immediately) is exactly what the site promises: it runs to the end of what
 * they paid for; on a trial, the period end IS the trial end, so they pay
 * nothing.
 */
async function cancelAtPeriodEnd(account) {
  if (!account || !account.stripeSubscriptionId) throw new Error('No active subscription to cancel.');
  const sub = await stripePost(`subscriptions/${account.stripeSubscriptionId}`, { cancel_at_period_end: 'true' });
  const endsAt = sub.current_period_end ? new Date(sub.current_period_end * 1000).toISOString() : '';
  db.updateAccount(account.id, { cancelAt: endsAt });
  db.logActivity(account.id, { agent: 'SYSTEM', msg: `Cancelled by the customer — plan runs until ${endsAt ? new Date(endsAt).toLocaleDateString() : 'the end of the period'}, nothing further will be charged` });
  // Trust the account's own record as well as Stripe's reply: the trial
  // wording ("your card will not be charged") must appear for anyone whose
  // trial this is, even if the API response lags a state change.
  return { endsAt, trialing: sub.status === 'trialing' || account.subStatus === 'trialing' };
}
/** Undo a pending cancellation. */
async function resumeSubscription(account) {
  if (!account || !account.stripeSubscriptionId) throw new Error('No subscription to resume.');
  await stripePost(`subscriptions/${account.stripeSubscriptionId}`, { cancel_at_period_end: 'false' });
  db.updateAccount(account.id, { cancelAt: '' });
  db.logActivity(account.id, { agent: 'SYSTEM', msg: 'Cancellation withdrawn — the plan continues' });
  return { ok: true };
}

/**
 * One-time top-up purchase: leads or minutes added to the account's balance.
 * mode:'payment' with an inline price, so no Stripe price objects to manage
 * and no interaction with the subscription -- a top-up can never double-bill
 * or touch the plan.
 */
async function createTopupCheckout(account, kind) {
  const t = plans.TOPUPS[kind];
  if (!t) throw new Error('No such top-up.');
  const base = process.env.PUBLIC_URL || 'http://localhost:8090';
  const session = await stripePost('checkout/sessions', {
    mode: 'payment',
    'line_items': [{ quantity: 1, price_data: { currency: 'usd', unit_amount: t.price * 100, product_data: { name: `Dawnpipe top-up — ${t.name}`, tax_code: TAX_CODE_SAAS } } }],
    success_url: `${base}/app?topup_session={CHECKOUT_SESSION_ID}`,
    cancel_url: `${base}/app`,
    client_reference_id: account.id,
    ...(account.stripeCustomerId ? { customer: account.stripeCustomerId } : { customer_email: account.email }),
    metadata: { topup: kind, account: account.id },
  });
  return session.url;
}
/**
 * Credit a paid top-up, idempotent on the Checkout Session id. The webhook
 * and the reconcile safety net can both fire for the same account close
 * together (or two top-ups can land back to back); topupLeads/topupMinutes
 * are additive off whatever the caller last read, so without serializing,
 * the second write can stomp the first before it's ever persisted — a paid
 * top-up silently not credited. withLock forces every credit for one account
 * through in order, and re-reads fresh inside the lock instead of trusting
 * the (possibly stale) account object the caller captured before its awaits.
 */
function creditTopup(account, kind, sessionId) {
  const t = plans.TOPUPS[kind];
  if (!t || !account) return Promise.resolve(false);
  return db.withLock(`account:${account.id}`, () => {
    const fresh = db.getAccount(account.id) || account;
    const seen = Array.isArray(fresh.topupSessions) ? fresh.topupSessions : [];
    if (sessionId && seen.includes(sessionId)) return false;
    db.updateAccount(fresh.id, {
      topupLeads: Math.max(0, Number(fresh.topupLeads || 0)) + t.leads,
      topupMinutes: Math.max(0, Number(fresh.topupMinutes || 0)) + t.minutes,
      topupSessions: [...seen, sessionId].slice(-50),
    });
    db.logActivity(fresh.id, { agent: 'SYSTEM', msg: `Top-up credited: ${t.name} ($${t.price})` });
    return true;
  });
}
/** The reconcile path for a top-up whose webhook was late or lost. */
async function reconcileTopup(account, sessionId) {
  if (!account || !/^cs_[A-Za-z0-9_]+$/.test(String(sessionId || ''))) return false;
  const key = process.env.STRIPE_SECRET_KEY;
  if (!key) return false;
  const r = await fetch(`https://api.stripe.com/v1/checkout/sessions/${encodeURIComponent(sessionId)}`, {
    headers: { Authorization: `Bearer ${key}` }, signal: AbortSignal.timeout(10000) });
  const sess = await r.json().catch(() => null);
  if (!r.ok || !sess) return false;
  if (sess.payment_status !== 'paid' || sess.client_reference_id !== account.id) return false;
  const kind = sess.metadata && sess.metadata.topup;
  if (!kind) return false;
  return creditTopup(db.getAccount(account.id) || account, kind, sess.id);
}
...
function applyEvent(event) {
  ...
  if (event.type === 'checkout.session.completed' && o.mode === 'payment' && o.metadata && o.metadata.topup) {
    const acc = db.getAccount(o.client_reference_id);
    if (acc) creditTopup(acc, o.metadata.topup, o.id);
    return;
  }
  if (event.type === 'checkout.session.completed') {
    ...
    if (acc) {
      ...
      // Keep the pending-cancellation flag in truth with Stripe, whichever
      // side flipped it (our button, the portal, or support).
      if (!event.type.endsWith('deleted')) {
        db.updateAccount(acc.id, { cancelAt: o.cancel_at_period_end && o.current_period_end ? new Date(o.current_period_end * 1000).toISOString() : '' });
      }
...
module.exports = { configured, configReport, hasAccess, canSpend, createPortal, isPaid, isOwner, createCheckout, createMissingPrices, backfillCommissions, cancelAtPeriodEnd, resumeSubscription, createTopupCheckout, creditTopup, reconcileTopup, ensureTaxCodes, verifyWebhook, applyEvent, inGrace, reconcileCheckout, DUNNING };
