...
function applyEvent(event) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (acc) db.updateAccount(acc.id, { stripeCustomerId: o.customer, subStatus: 'active', plan: 'pro', tier });
    ...
    if (acc) {
      ...
      const t = (o.metadata && o.metadata.tier)
        || (o.items && o.items.data && o.items.data[0] && plans.tierForPriceId(o.items.data[0].price && o.items.data[0].price.id));
      if (t) patch.tier = t;
...
module.exports = { configured, configReport, hasAccess, isPaid, isOwner, createCheckout, verifyWebhook, applyEvent };
