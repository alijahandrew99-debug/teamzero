...
async function createTopupCheckout(account, kind) {
  ...
  const session = await stripePost('checkout/sessions', {
    mode: 'payment',
    'line_items': [{ quantity: 1, price_data: { currency: 'usd', unit_amount: t.price * 100, product_data: { name: `Dawnpipe top-up — ${t.name}` } } }],
...
module.exports = { configured, configReport, hasAccess, canSpend, createPortal, isPaid, isOwner, createCheckout, createMissingPrices, backfillCommissions, cancelAtPeriodEnd, resumeSubscription, createTopupCheckout, creditTopup, reconcileTopup, verifyWebhook, applyEvent, inGrace, reconcileCheckout, DUNNING };
