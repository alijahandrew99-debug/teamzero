...
async function send(to, subject, body, attachment) {
  if (!enabled()) return { ok: false, error: 'system mailbox not configured' };
  try {
    await smtp.sendMail(cfg(), { to, subject, body, attachment: attachment || null });
    return { ok: true };
  } catch (e) {
...
// The system mailbox config, so a customer who chooses "let Dawnpipe send for
// me" can route their outreach through it without ever touching credentials.
function systemCfg() { return cfg(); }
module.exports = { enabled, send, sendPasswordReset, sendMorningDigest, systemCfg };
