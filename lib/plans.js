...
const TIERS = {
  ...
  complete: { id: 'complete', name: 'Complete', price: 1499, leads: 2000, voiceMinutes: 1000, priceEnvKey: 'STRIPE_PRICE_COMPLETE',
              blurb: 'Email + the AI answers and makes your calls' },
};
const TIER_ORDER = ['tester', 'starter', 'growth', 'scale', 'complete'];
...
function usage(account, owner = false) {
  ...
  if (pro && account.usagePeriod !== periodKey()) vUsed = 0;
...
function consumeVoiceMinutes(account, minutes, owner = false) {
  ...
  const key = periodKey();
  const rolled = account.usagePeriod !== key;
  const used = (rolled ? 0 : Number(account.voiceMinutesUsed || 0)) + minutes;
  db.updateAccount(account.id, { voiceMinutesUsed: used, usagePeriod: key });
}
