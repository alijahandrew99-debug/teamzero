...
function consumeVoiceMinutes(account, minutes, owner = false) {
  ...
  const before = rolled ? 0 : Number(account.voiceMinutesUsed || 0);
  ...
  const tier = TIERS[accountTier(account)] || {};
  const planLimit = account.subStatus === 'trialing' ? Math.min(tier.voiceMinutes || 0, TRIAL_VOICE_MINUTES) : (tier.voiceMinutes || 0);
  const draw = Math.max(0, used - planLimit) - Math.max(0, before - planLimit);
  const patch = { voiceMinutesUsed: used, voicePeriod: key };
  if (draw > 0) patch.topupMinutes = Math.max(0, Number(account.topupMinutes || 0) - draw);
...
function consume(account, n, owner = false) {
  ...
  const used = before + n;
  const planLimit = pro ? (TIERS[accountTier(account)] || {}).leads || 0 : TRIAL_LEADS;
  const draw = Math.max(0, used - planLimit) - Math.max(0, before - planLimit);
  const patch = { leadsUsed: used, usagePeriod: key };
  if (draw > 0) patch.topupLeads = Math.max(0, Number(account.topupLeads || 0) - draw);
  db.updateAccount(account.id, patch);
}
