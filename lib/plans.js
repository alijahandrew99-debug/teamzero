...
const TIERS = {
  // FOUR plans, one story: "your phone is answered and your pipeline is full".
  // Three carry the phone; one is the downsell for a buyer who genuinely only
  // wants leads. Every plan clears 43% margin at FULL allowance use, so none of
  // them depends on customers under-using what they bought.
  //
  // Deliberately NOT priced per minute. Competitors sell minutes and race each
  // other down ($149 for 1,000 is out there); nobody else bundles the outbound
  // half, so that is the ground we compete on. Minutes here are a cost control,
  // not the product.
  //
  // Two plans were removed rather than repriced. Starter at $99 paid a
  // commission rep $29.70 — no one prospects a plumber for that, so it could
  // never be sold by the channel this business runs on. Scale at $699 sold
  // 2,000 leads with no phone, which is Complete minus the reason to buy
  // Complete; it existed only to confuse the ladder.
  // Key stays 'growth' because that is the metadata.tier on the live Stripe
  // price; renaming a plan must not orphan the price that already sells it.
  growth:   { id: 'growth',   name: 'Outreach',  price: 299,  leads: 750,  voiceMinutes: 0,    priceEnvKey: 'STRIPE_PRICE_GROWTH',
              group: 'leads', trial: true,
              blurb: 'New customers found and written to, every day',
              tagline: 'For when the phone is covered and the pipeline is not.',
              who: 'For the business that already handles its own calls but wants a steady stream of new customers arriving.',
              features: [
                '750 verified leads a month — roughly 25 new prospects every working day',
                'A personal email drafted for every one, in your voice',
                'You approve every send; nothing goes out without you',
                'Overnight runs: wake up to a queue, not a blank screen',
                'Domain warmup and paced sending that protect your reputation',
              ] },
  frontdesk:{ id: 'frontdesk',name: 'Front Desk',price: 499, leads: 200,  voiceMinutes: 500,  priceEnvKey: 'STRIPE_PRICE_FRONTDESK',
              ...
              who: 'For the single-location business losing jobs to voicemail — the plumber on a roof, the salon mid-appointment, the shop with one phone and no one free to answer it.',
              features: [
  ...
  pro:      { id: 'pro',      name: 'Front Desk Pro', price: 899, leads: 750, voiceMinutes: 1200, priceEnvKey: 'STRIPE_PRICE_PRO',
              group: 'phone', trial: true,
              blurb: 'For a busy shop, or several of them',
              tagline: 'The front desk that keeps up with a real phone.',
              who: 'For the multi-van trade, the busy clinic, or anyone whose phone rings all day and who wants the outbound side working just as hard.',
              features: [
                'Everything in Front Desk',
                '1,200 talk-minutes a month — for a line that genuinely rings all day',
                '750 new customers found and written to each month, not 200',
                'Webhooks and API access: plug it into your CRM or Zapier',
                'Priority support from a real person',
              ] },
  complete: { id: 'complete', name: 'Complete', price: 1499, leads: 2000, voiceMinutes: 1500, priceEnvKey: 'STRIPE_PRICE_COMPLETE',
              ...
              who: 'For multi-location businesses and agencies that want every call answered and the pipeline full, at scale.',
              features: [
                'Everything in Front Desk Pro',
                '1,500 talk-minutes a month',
                '2,000 new customers found and written to each month',
                'Webhooks and API access: plug it into your CRM or Zapier',
                'Priority support from a real person',
              ] },
};
const TIER_ORDER = ['frontdesk', 'pro', 'complete', 'growth'];
...
function catalogue() {
  return tierList().map((t) => {
    ...
    const cents = id ? priceAmounts[id] : undefined;
    const charged = (cents === undefined || cents === null) ? t.price : Math.round(cents / 100);
    return { ...t, price: charged, listPrice: t.price, available: !!id, priceCurrent: priceIsCurrent(t.id) };
  });
}
