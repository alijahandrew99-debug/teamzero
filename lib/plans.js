...
const TIERS = {
  ...
  //
  // 500 minutes, not 1,000, and the reason is margin SHAPE rather than margin
  // level. Voice is ~92% of what this plan costs to deliver, so at 1,000
  // included minutes the gap between a light customer and a heavy one was
  // ~$103/mo — profit depended on customers NOT using what they bought, which
  // is a bet, not a business. At 500 that spread is ~$24, so the plan earns
  // about the same whoever buys it. 500 minutes is roughly 150-250 answered
  // calls, comfortably above what a single-location trade actually uses.
  frontdesk:{ id: 'frontdesk',name: 'Front Desk',price: 499, leads: 200,  voiceMinutes: 500,  priceEnvKey: 'STRIPE_PRICE_FRONTDESK',
              group: 'phone', trial: true, badge: 'Most popular', legacyPrices: [399, 449],
              ...
              features: [
                ...
                '500 talk-minutes a month — around 150 to 250 answered calls',
...
// ---- top-up packs ----
// The answer to "can I buy Front Desk AND Scale?" -- which the billing model
// deliberately forbids (one subscription per account, so nobody can be
// double-billed). Plans do not stack; a top-up is a ONE-TIME purchase that
// adds to the account's balance and is drawn down only after the month's
// plan allowance is spent. Unused balance carries over. Never automatic,
// never recurring, never called overage -- "no overage charges, ever" stays
// exactly true because only the customer can buy one, on purpose.
// PRICED so subscribing is always the better per-unit deal: 2,000 top-up
// leads cost the same $699 as a month of Scale, which gives 2,000 EVERY
// month -- so a spike buys a pack, and a pattern upgrades the plan.
const TOPUPS = {
  minutes250: { id: 'minutes250', name: '+250 phone minutes', price: 79,  minutes: 250, leads: 0 },
  leads500:   { id: 'leads500',   name: '+500 leads',         price: 199, minutes: 0,   leads: 500 },
  leads2000:  { id: 'leads2000',  name: '+2,000 leads',       price: 699, minutes: 0,   leads: 2000 },
};
...
/**
 * Tiers as the SITE should show them.
 *
 * `price` is what a customer will actually be charged — read back from the
 * Stripe price we would send them to, not from the number in TIERS. Those two
 * are the same in the steady state, and differ only in the window between
 * deciding on a new price and creating it in Stripe. In that window the site
 * must quote what the card will be debited: a page saying $449 over a $399
 * charge is wrong even though it is wrong in the customer's favour, and it
 * would also pay the referring rep a commission computed off a price nobody
 * paid. `listPrice` keeps the intended number for the owner's view.
 */
function catalogue() {
  return tierList().map((t) => {
    const id = priceIdForTier(t.id);
    //  is the number the OWNER set, full stop. Checkout guarantees the
    // customer is charged exactly that (it creates the Stripe price on demand
    // if it does not exist yet), so the site never has to hedge. 
    // is true whenever billing is configured, for the same reason: with a
    // Stripe key, every tier is sellable, because the price will exist by the
    // time anyone reaches the pay button.
    return { ...t, price: t.price, listPrice: t.price,
      available: !!id || !!process.env.STRIPE_SECRET_KEY, priceCurrent: priceIsCurrent(t.id) };
  });
}
...
const priceAmounts = {}; // priceId -> unit_amount in cents, for honest checks
const missingTaxCode = new Set(); // Dawnpipe product ids with no tax category
let discoveredAt = 0;
async function discoverPrices() {
  ...
  for (let guard = 0; url && guard < 5; guard++) {
    ...
    for (const p of data.data || []) {
      ...
      priceAmounts[p.id] = Number(p.unit_amount);
      const t = TIERS[tier];
      if (!t) continue;
      // Tax-category audit happens HERE, before the amount checks: a
      // legacy-priced product (the old $399/$449 Front Desk) exits this loop
      // early as "legacy", but it still carries live subscriptions and needs
      // a tax category just as much as the current one. The category only
      // gates Stripe's opt-in Managed Payments -- checkout works without it
      // -- but the dashboard's "Needs info" badge reads like something is
      // broken, so the boot sync clears it for every Dawnpipe product.
      {
        const pr = p.product && typeof p.product === 'object' ? p.product : null;
        if (pr && pr.active !== false && /dawnpipe/i.test(pr.name || '') && !pr.tax_code) missingTaxCode.add(pr.id);
      }
...
function productsMissingTaxCode() { return [...missingTaxCode]; }
function clearMissingTaxCode(id) { missingTaxCode.delete(id); }
/** What each tier is ACTUALLY charging right now, in cents. For diagnostics. */
function chargedAmounts() {
  const out = {};
  for (const k of TIER_ORDER) { const id = priceIdForTier(k); out[k] = { priceId: id, cents: id ? (priceAmounts[id] ?? null) : null, listed: TIERS[k].price * 100 }; }
  return out;
}
...
function priceIdForTier(tierId) {
  ...
  // Precedence is by CORRECTNESS, not by source. An env var is an escape
  // hatch, not an authority: a STRIPE_PRICE_* still pointing at last month's
  // price must not beat a correct price discovered from Stripe, or the site
  // quotes $449 and the card is debited $399 -- which is exactly what happened.
  // So: an env price that charges the right amount wins, then a discovered one
  // (correct by construction), then the env price anyway, then legacy.
  const env = process.env[t.priceEnvKey] || '';
  const want = t.price * 100;
  if (env && priceAmounts[env] === want) return env;
  if (discovered[tierId]) return discovered[tierId];
  if (env) return env;
  return (legacyIds[tierId] || [])[0] || '';
}
/** Is this tier being sold at its CURRENT price, or still on a legacy one? */
function priceIsCurrent(tierId) {
  const t = TIERS[tierId];
  if (!t) return true;
  const id = priceIdForTier(tierId);
  if (!id) return false;
  // Check the AMOUNT, not which slot the id came from. A STRIPE_PRICE_* env var
  // pinned to last month's price would otherwise report "current" while
  // charging the old number -- the page says $449, the card is debited $399.
  // If we have not seen the price in discovery we cannot vouch for it, so an
  // env-pinned id we know nothing about is reported as not-current rather than
  // assumed fine.
  const amount = priceAmounts[id];
  if (amount === undefined) return !process.env[t.priceEnvKey] && !!discovered[tierId];
  return amount === t.price * 100;
}
...
function usage(account, owner = false) {
  ...
  // Top-up balances sit ON TOP of the plan allowance and are drawn down (in
  // consume/consumeVoiceMinutes) only once the plan's own allowance is spent,
  // so an unused pack simply waits for the month it is needed.
  const topupLeads = Math.max(0, Number(account.topupLeads || 0));
  const topupMinutes = Math.max(0, Number(account.topupMinutes || 0));
  // The balance was already DECREMENTED for any usage beyond the plan line
  // (see consume), so the overflow has to be added back into the ceiling or
  // drawn usage would count against the pack twice and remaining would read
  // low. With it: remaining = plan-left + pack when under the line, and
  // exactly the pack when over it.
  const leadOverflow = Math.max(0, used - limit);
  const minOverflow = Math.max(0, vUsed - vLimit);
  return {
    plan: pro ? 'pro' : 'trial',
    tier,
    tierName: pro ? TIERS[tier].name : 'Free trial',
    used,
    limit: limit + topupLeads + leadOverflow,
    remaining: Math.max(0, limit + topupLeads + leadOverflow - used),
    topupLeads, topupMinutes,
    unlimited: false,
    isPro: pro,
    voice: { used: vUsed, limit: vLimit + topupMinutes + minOverflow, remaining: Math.max(0, vLimit + topupMinutes + minOverflow - vUsed), included: (vLimit + topupMinutes) > 0 },
  };
}

/** Record phone minutes against the plan's allowance, then the top-up balance. */
function consumeVoiceMinutes(account, minutes, owner = false) {
  if (owner || !minutes) return;
  // Re-read rather than trusting the caller's snapshot. This is a
  // read-modify-write on a counter, and callers reach it across an `await` --
  // so a snapshot taken before that await has already been overtaken by
  // whatever else metered this account in the meantime, and writing
  // `snapshot + n` silently erases it. db.js is fully synchronous, so a
  // re-read and the write below cannot interleave: this makes the whole
  // update atomic. Same reasoning as agents.js's per-iteration re-read and
  // the sentToday re-read in server.js.
  const cur = db.getAccount(account.id) || account;
  const key = periodKey();
  const rolled = cur.voicePeriod !== key;
  const before = rolled ? 0 : Number(cur.voiceMinutesUsed || 0);
  const used = before + minutes;
  // Usage beyond the plan's own allowance draws down the top-up balance —
  // exactly the amount that crossed the line this call, never double-counted.
  const tier = TIERS[accountTier(cur)] || {};
  const planLimit = cur.subStatus === 'trialing' ? Math.min(tier.voiceMinutes || 0, TRIAL_VOICE_MINUTES) : (tier.voiceMinutes || 0);
  const draw = Math.max(0, used - planLimit) - Math.max(0, before - planLimit);
  const patch = { voiceMinutesUsed: used, voicePeriod: key };
  if (draw > 0) patch.topupMinutes = Math.max(0, Number(cur.topupMinutes || 0) - draw);
  db.updateAccount(account.id, patch);
}
...
function voiceAllowed(account, owner = false) {
  ...
  if (u.voice.remaining <= 0) return { ok: false, reason: `You've used all ${u.voice.limit} phone minutes this month. They reset next month — or add a one-time top-up pack from the plan screen if you need more now.` };
  return { ok: true, remaining: u.voice.remaining };
}

/** Record leads consumed, rolling the period for paid plans, then the top-up balance. */
function consume(account, n, owner = false) {
  if (owner || !n) return;
  // Re-read for the same reason consumeVoiceMinutes does — see the note there.
  // The night shift makes this concrete: server.js fires keeperRun and the
  // prospect job for the same account back to back, and both meter leads.
  // keeperRun reads the account, awaits a draft for several seconds, then
  // writes `snapshot + 1`, wiping every lead the prospect run metered during
  // that await.
  const cur = db.getAccount(account.id) || account;
  const pro = isPro(cur);
  const key = periodKey();
  const rolled = pro && cur.usagePeriod !== key;
  const before = rolled ? 0 : Number(cur.leadsUsed || 0);
  const used = before + n;
  const planLimit = pro ? (TIERS[accountTier(cur)] || {}).leads || 0 : TRIAL_LEADS;
  const draw = Math.max(0, used - planLimit) - Math.max(0, before - planLimit);
  const patch = { leadsUsed: used, usagePeriod: key };
  if (draw > 0) patch.topupLeads = Math.max(0, Number(cur.topupLeads || 0) - draw);
  db.updateAccount(account.id, patch);
}

module.exports = {
  usage, consume, consumeVoiceMinutes, voiceAllowed, isPro, TRIAL_LEADS, periodKey,
  TIERS, TIER_ORDER, TRUST, TOPUPS, tierList, catalogue, priceIdForTier, priceIsCurrent, tierForPriceId, accountTier, cheapestTier, fromPrice,
  discoverPrices, discoveredPrices, chargedAmounts, productsMissingTaxCode, clearMissingTaxCode,
};
