...
const F = {
  ...
  funnel: path.join(DATA, 'funnel.json'),
};
...
function logSend(accountId, rec) {
  ...
  write(F.sendlog, all);
  trackFunnel(accountId, 'first_send');
}
...
function addLeads(accountId, profileId, rows) {
  ...
  write(F.leads, all);
  if (added.length) trackFunnel(accountId, 'first_lead');
  return added;
}
...
// ---------------- activation funnel ----------------
// No third-party tracker: one row per account per milestone, recorded the
// first time that account crosses it. Repeat calls (e.g. every send) are
// no-ops, so callers can call this unconditionally without double-counting.
const FUNNEL_STEPS = ['signup', 'profile', 'first_lead', 'first_send', 'upgrade'];
function trackFunnel(accountId, step) {
  if (!FUNNEL_STEPS.includes(step)) return null;
  const all = read(F.funnel, []);
  if (all.some((f) => f.accountId === accountId && f.step === step)) return null;
  const row = { id: uid(), accountId, step, at: nowISO() };
  all.push(row);
  write(F.funnel, all);
  return row;
}
function getFunnelSummary() {
  const all = read(F.funnel, []);
  const counts = {};
  for (const step of FUNNEL_STEPS) counts[step] = all.filter((f) => f.step === step).length;
  return counts;
}
...
module.exports = {
  ...
  trackFunnel, getFunnelSummary, FUNNEL_STEPS,
};
