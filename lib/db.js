...
function accountByVoiceNumber(number) {
  ...
  const hit = all.find((a) => a.voice && a.voice.number && a.voice.number.replace(/[^0-9]/g, '') === n);
  if (hit) return hit;
  // Fall back to the shared env number ONLY while exactly one account has voice
  // switched on. With two or more, an inbound call previously went to whichever
  // account was created first — one tenant answering another tenant's calls.
  const configured = all.filter((a) => a.voice && a.voice.enabled);
...
module.exports = {
  ...
  saveCall, getCalls, accountByVoiceNumber, accountByCallSid,
