...
// Every write above is a synchronous read-modify-write, so within ONE call the
// tmp+rename above can't be interrupted by another. The real race lives at the
// CALLER: read an account, `await` a Stripe round trip, then write a patch
// computed from that now-stale read — two concurrent webhook/reconcile paths
// for the same account can clobber each other (a top-up credited twice, or
// dropped). withLock serializes same-key async sequences so a caller can
// re-read fresh data once it holds the lock and know nothing else is
// interleaved. Keyed by whatever the caller chooses (e.g. `account:<id>`).
const locks = new Map();
