...
function write(f, d) {
  // Unique per call, not just per file: a fixed '.tmp' name means two writers
  // sharing DATA_DIR (e.g. an overlapping Render deploy) race on the same temp
  // path, and the loser's renameSync can splice its own write over the
  // winner's, or land after it and get overwritten in turn -- either way the
  // interleaving is silent. Uniqueness removes the collision regardless of
  // whether that overlap is actually possible today.
