...
function saveCall(accountId, rec) {
  ...
  write(F.calls, capPerAccountOldestFirst(all));
  return rec;
}
...
function saveThread(accountId, phone, turns, patch = {}) {
  ...
  write(F.smsThreads, capPerAccountOldestFirst(all));
  return row;
}
...
// Same idea, for collections that push (oldest-first) instead of unshift
// (newest-first) — `calls` and `smsThreads`. Walking forward like capPerAccount
// does would keep the OLDEST N per account here and evict everything recent, so
// this walks backward and keeps the newest N instead, then restores oldest-first
// order for the writers that expect it.
function capPerAccountOldestFirst(rows, perAccount = 500) {
  const seen = new Map();
  const kept = [];
