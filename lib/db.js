...
function updateCallback(accountId, id, patch) {
  ...
  if (i < 0) return null;
  // Consent evidence is append-only: the wording, timestamp and IP a person
  // consented under can never be edited, or it stops being evidence.
  const { consentText, consentAt, consentIp, at, accountId: _a, id: _i, ...safe } = patch || {};
  all[i] = { ...all[i], ...safe };
  write(F.callbacks, all);
  return all[i];
}
