...
// Bulk-clear leads for a profile. mode 'all' wipes them; 'guesses' keeps only
// verified/real addresses and drops pattern/invalid/role guesses. Cascades to
// any queued drafts for the removed leads.
function clearLeads(accountId, profileId, mode = 'all') {
  const all = read(F.leads, []);
  const GUESS = new Set(['pattern', 'invalid', 'role', 'site', '']);
  const isGuess = (l) => GUESS.has((l.emailConfidence || '').toLowerCase());
  const removed = [];
...
module.exports = {
  ...
  getLeads, addLeads, updateLead, clearLeads,
