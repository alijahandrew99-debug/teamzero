...
const VOICES = [
  ...
  { value: 'Polly.Lupe-Neural',           label: 'Lupe — Spanish (US), warm female',      tier: 'neural', lang: 'es-US' },
  { value: 'Polly.Pedro-Neural',          label: 'Pedro — Spanish (US), male',            tier: 'neural', lang: 'es-US' },
...
const RATE_VOICE_MIN = Number(process.env.VOICE_RATE_MIN || 0.014);      // per minute (outbound rate; inbound is cheaper)
const RATE_RECOGNITION = Number(process.env.VOICE_RATE_GATHER || 0.02);  // per gather
const RATE_AI_TURN = Number(process.env.VOICE_RATE_AI || 0.006);         // Haiku, prompt cached, per turn
const RATE_RECORDING_MIN = 0.0025;
function estimateCost({ durationSec = 0, turns = 0, chars = 0, tier = 'generative', recorded = true }) {
  ...
  return Number((mins * RATE_VOICE_MIN + gathers * RATE_RECOGNITION + gathers * RATE_AI_TURN + tts + (recorded ? mins * RATE_RECORDING_MIN : 0)).toFixed(3));
}
