...
// most maddening thing an AI phone agent can do. Two seconds is roughly the
// gap a person waits before assuming you've finished; it costs ~2s of latency
// per turn and that is a trade worth making every time.
const SPEECH_TIMEOUT = process.env.VOICE_SPEECH_TIMEOUT || '2';
// Which speech model Twilio uses, and it is a PRICING decision as much as an
// accuracy one: Twilio's default ("Twilio picks provider", and it can fail
// over between providers) bills $0.02 per use, while the legacy experimental_*
// / Google-v1 models bill $0.035-$0.040 per use for the same job. Because
// recognition bills per USE, that gap is ~$45-60 per 1,000 talk-minutes.
// Set VOICE_STT=default (or 'none'/'off') to drop the attribute entirely and
// take Twilio's default. /api/admin/spend reports which rate we are actually
// being charged, so this is decidable from the bill rather than from docs.
const SPEECH_MODEL = process.env.VOICE_STT || 'experimental_conversations';
const USE_SPEECH_MODEL = !/^(default|none|off|)$/i.test(String(process.env.VOICE_STT ?? 'experimental_conversations').trim());
