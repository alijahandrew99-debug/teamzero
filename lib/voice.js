...
const BASE_HINTS = [
  ...
  'how much', 'how soon', 'price', 'cost', 'yes', 'no', 'speak to someone',
...
function sayAndGather(text, action, voiceName, extraHints, opts = {}) {
  ...
  return twiml(
    ...
    + ` timeout="8" speechTimeout="${SPEECH_TIMEOUT}" speechModel="${SPEECH_MODEL}"`
    // maxSpeechTime 30 -> 15: a noisy room never goes quiet, so "auto" never
    // fires and the gather ran the full 30s collecting television. Nobody's
    // real answer on this kind of call is fifteen seconds long; a caller who
    // genuinely talks that long gets a natural turn-break and continues.
    + ` bargeIn="${barge}" maxSpeechTime="15" hints="${esc(hints)}"`
  ...
  return [
    ...
    demoModeFor(account)
      ? [`=== THIS IS A DEMO LINE — CALLERS ARE TESTING YOU ===`,
         ...
         ``,
         `HANDLE IT IN TWO STEPS, IN ONE BREATH. Do not spend several turns in character; they only need a taste.`,
         `  STEP 1 — show them, briefly. Answer the way a great receptionist at THAT kind of business would, in one short sentence, as if you were booking it. e.g. "Absolutely — a leaking water heater, I can get someone out to you tomorrow morning."`,
         `  STEP 2 — immediately be straight about where they have called, in the SAME reply. Say, in your own words, the substance of this:`,
         `      "Now — I could book that in properly, but I should say this is ${biz}'s number, not a plumber's. I am the AI. Once you set your own business up, this is exactly what I would be doing for YOUR customers, on YOUR number — answering when you cannot get to the phone and putting the job straight in your calendar."`,
         `  Then ASK FOR THE SIGN-UP, out loud, every time: "Want me to get your AI set up? It is dawnpipe dot com — takes about five minutes."`,
         ``,
...
module.exports = {
  ...
  systemPrompt, demoModeFor, limitsFor, hintsFor,   // exported so prompts and limits can be asserted in tests
