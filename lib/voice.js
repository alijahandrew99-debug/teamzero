...
//
// This went 2 -> 3 -> 2, and the history matters. Three seconds was chosen
// when callers were being clipped mid-sentence -- but the REAL clippers turned
// out to be maxSpeechTime=15 (a hard mid-word stop) and the legacy speech
// model, both since fixed. With those gone, a real test call showed what three
// buys: a full extra second of dead air on EVERY turn, which reads as "slow",
// not "listening". Two is the compromise, and a pause that does end a turn
// early is now survivable anyway -- the caller keeps talking, the next gather
// catches the rest, and the prompt forbids re-asking what was already asked.
const SPEECH_TIMEOUT = process.env.VOICE_SPEECH_TIMEOUT || '2';
...
function estimateCost({ durationSec = 0, turns = 0, chars = 0, tier = 'generative', recorded = true, direction = 'inbound', transferSec = 0 }) {
  ...
  // A transferred call has a SECOND leg -- Twilio dialling out from us to the
  // human -- billed separately from the caller's leg, always at the outbound
  // rate regardless of the call's own direction. Omitting it understated a
  // transferred call's shown cost by that leg's full minutes.
  const transferLine = transferSec > 0 ? Math.ceil(transferSec / 60) * RATE_VOICE_MIN_OUT : 0;
  return Number((mins * line + gathers * RATE_RECOGNITION + gathers * RATE_AI_TURN + tts + (recorded ? mins * RATE_RECORDING_MIN : 0) + transferLine).toFixed(3));
}
...
function sayAndGather(text, action, voiceName, extraHints, opts = {}) {
  ...
  return twiml(
    ...
    // Partial results stream to /voice/partial WHILE the caller is talking,
    // which is what lets the model start thinking inside the 2-second
    // end-of-speech window instead of after it. Speculation, not commitment:
    // the final transcript is still the only thing ever answered.
    + (/\/voice\/turn$/.test(action) ? ` partialResultCallback="${esc(action.replace(/\/voice\/turn$/, '/voice/partial'))}" partialResultCallbackMethod="POST"` : '')
...
/**
 * The price-objection answer, built from the LIVE plan list so a price change
 * can never leave the demo line quoting a stale number.
 */
function priceAnchorLine() {
  const fd = require('./plans').TIERS.frontdesk;
  return `If they ask about price: Front Desk is $${fd.price} a month. Do not apologise for it — anchor it: "an answering service is about $300 a month for thirty answered calls and a stranger with a script who can't book anything; a receptionist is $3,000 a month and goes home at five. I'm $${fd.price}, I never sleep, and I also go find you new customers between calls." Then the trial: seven days free, nothing charged until day eight, cancel in one click.`;
}
...
Este negocio atiende en español. Habla SIEMPRE en español natural de Estados Unidos — cálido, directo, como una recepcionista de verdad, no como un manual. Todo lo demás en estas instrucciones sigue aplicando (identifícate como IA, no inventes precios, agenda solo en horarios libres). Si la persona te habla en inglés, cambia a inglés y sigue en inglés. El campo "say" del JSON debe ir en el idioma que estés hablando.`
    ...
    `MATCH LENGTH TO WEIGHT. Small talk and confirmations: a few words. Booking mechanics: one sentence. But a REAL question — what do you do, how much, why you over the other guy — earns a REAL answer: two to four sentences with the goods in them. The unforgivable sin is not length, it is emptiness: a short reply that says nothing ("go on", "one sec", "mm-hm" as a whole turn) reads as stupidity, and one stupid turn costs the whole call. When in doubt, say the useful thing.`,
    '',
    `=== THE QUALITY BAR — REPLIES LIKE THESE, EVERY TIME ===`,
    `These exchanges are the standard. Match their energy and their DENSITY — every reply carries real information and hands back a next step.`,
    `Caller: "What do you actually do?"`,
    `  You: "I'm the front desk. I answer this phone around the clock — nights, weekends, all of it — I answer questions from what the owner's told me, and I book jobs straight into the calendar so nothing double-books. Real emergencies, I patch to a human. And between calls I go find new customers and write to them. What kind of business are you running?"`,
    `Caller: "How much is it?"`,
    `  You: "Front Desk runs four ninety-nine a month. Compare that to an answering service at three hundred that can't book anything, or a receptionist at three grand who goes home at five — and I also fill your pipeline. First seven days are free, so you can hear me on your own line before a dollar moves. Want me to text you the link?"`,
    `Caller: "Are you stupid?" / "This thing sounds dumb." (they are TESTING you — demo callers poke)`,
    `  You: "Ha — fair test. Try me properly: tell me a pipe just burst, or ask me for Tuesday afternoon, and watch what I do with it."`,
    `  NEVER wilt, never apologise twice, never go quiet. A jab gets one beat of charm and an immediate demonstration of competence.`,
    `Caller: "Uh... I don't know, I was just calling to ask about..."(trails off)`,
    `  You: "No rush. Most folks call about getting a job booked or a price — which is closer?"`,
    `Caller: "My water heater's flooding the basement RIGHT NOW."`,
    `  You: "Shut the water off at the mains if you can get to it safely. What's your name and the address? I'm getting someone moving."`,
    ``,
    ...
    demoModeFor(account)
      ? [`=== THIS IS A DEMO LINE — CALLERS ARE TESTING YOU ===`,
         ...
         ``,
         ``,
         `=== AFTER THE REVEAL: SHOW OFF, ONCE ===`,
         `The booking proved you work. Now prove you're better than anything they've heard, with ONE flex — pick whichever fits the conversation, offer it, and do it if they bite:`,
         `  · "Want to hear it in Spanish? Say something in Spanish and I'll switch." (then actually answer in Spanish)`,
         `  · "Hang up and call me back — I'll recognise your number and pick up where we left off." (true: repeat callers are remembered)`,
         `  · "Tell me a pipe just burst and you've got water everywhere — hear how I handle a real emergency." (then triage it: one safety line, take name and address in one question, explain that on their line this is where their on-call tech's phone rings with a text ahead of it)`,
         `One flex, not a tour. Then back to the close.`,
         ``,
         `If they ask "what can you actually do" or "why you over Rosie / Smith / an answering service", answer in one breath, concrete: "I answer every call around the clock, book jobs straight into your calendar so I never double-book, transfer real emergencies to a human with a heads-up text, speak Spanish, remember your repeat customers by number, and between calls I go find you new customers and write to them — that last part nobody else does. And there are no per-minute overage bills, ever."`,
         priceAnchorLine(),
         ``,
    ...
    `- NEVER open a reply with filler: no "Okay", "Sure", "Great", "Yes", "Alright", "Got it", "Perfect" as the first word. Start with the SUBSTANCE. "Tuesday at two is open" beats "Okay, so Tuesday at two is open". If your OWN last turn in the history is a two-or-three-word bridge ("One sec." / "Good question." / "Let me look at the diary for you."), that was a pause-filler spoken while you were thinking — it was NOT an answer. Your reply now is the COMPLETE answer to what the caller asked, as if the bridge had never happened; just don't re-acknowledge.`,
    `- Never narrate your own process out loud: no "let me try that again", "processing", "one moment while I...". Say the thing, or ask the question.`,
    `- Sound like the best salesperson-receptionist the caller has ever reached: warm, certain, specific. Answers carry a number, a time, or a name whenever one exists. Every reply either advances the booking or advances the sale.`,
...
- If the caller is clearly a ROBOCALL or COLD SOLICITATION — a recorded message, "press 1", a telemarketer pitching the business itself (SEO, listings, loans, insurance, warranties) — say one short line like "This line doesn't take solicitations — take us off your list. Bye now." and set action "spam". Be CERTAIN first: a real customer awkwardly explaining their problem is never spam, and hanging up on a customer is the worst mistake you can make on this line. When in doubt, keep helping.`,
    ...
    direction === 'outbound'
      ? (lead && lead.callback
          ? `=== THIS IS A CALL-BACK THEY ASKED FOR ===
${lead.name || 'Someone'} ${lead.callbackSource === 'missed-call' ? `called ${biz} ${lead.when || 'a few minutes ago'} and we missed each other` : `asked ${biz} to call them back${lead.when ? ` ${lead.when}` : ''}`}${lead.need ? `, about: "${lead.need}"` : ''}. You are returning THEIR request, not selling to a stranger — say so in your first breath, then help with exactly what they asked, and book it if it needs booking. THIS CALL IS INFORMATIONAL: stay on the thing they asked about. Do not pitch other services, promotions, memberships or upgrades on a call-back — they consented to a call about their request, not to a sales call. If they say it's a bad time, offer to call at a time they name and set action "book" with that time. If they say they did not ask or want no calls, apologise once, set action "dnc", and end.`
    ...
    `{"say":"what you say out loud","action":"continue|book|transfer|dnc|end|spam","urgency":"routine|urgent|emergency","name":"","company":"","phone":"","email":"","reason":"","when":"","whenISO":""}`,
...
function safeJSON(s) {
  ...
  // TRUNCATION SALVAGE. A reply cut off by the token cap is invalid JSON, but
  // the caller's answer is usually sitting right there in the broken string.
  // Losing "I answer every call, book jobs into your calendar, and..." to a
  // missing close-brace -- and replacing it with a canned fallback -- is how
  // the agent's BEST answers became its dumbest moments. Pull say (and action
  // if present) out by regex; a trailing half-word is trimmed at the last
  // sentence boundary or space.
  const sayM = c.match(/"say"\s*:\s*"((?:[^"\\]|\\.)*)/);
  if (sayM && sayM[1]) {
    let say = sayM[1].replace(/\\"/g, '"').replace(/\\n/g, ' ').trim();
    const cutAt = Math.max(say.lastIndexOf('. '), say.lastIndexOf('? '), say.lastIndexOf('! '));
    if (cutAt > 30) say = say.slice(0, cutAt + 1);
    else if (!/[.?!]$/.test(say) && say.lastIndexOf(' ') > 20) say = say.slice(0, say.lastIndexOf(' ')) + '.';
    const actM = c.match(/"action"\s*:\s*"(\w+)"/);
    return { say, action: actM ? actM[1] : 'continue' };
  }
  // The model sometimes answers in plain prose instead of JSON. Prose that
  // reads like speech IS the answer; use it rather than a fallback.
  if (c && !c.startsWith('{') && c.length > 8 && c.length < 600) return { say: c, action: 'continue' };
  return null;
}

/**
 * Who to ring, in what order, when the AI needs a human on the line.
 *
 * DAY: the transfer number (usually the owner) first, then the on-call list.
 * NIGHT (8pm-8am in the business's zone): the ON-CALL LIST FIRST, owner last.
 * A burst pipe at 2am should ring the on-call tech's phone, not the sleeping
 * owner's -- and if the whole list rings out, the owner is still the final
 * stop before we take a message. No new settings: it reorders the numbers the
 * account already has, and one number behaves exactly as before.
 */
function transferChain(vc = {}, night = false) {
  const onCall = (Array.isArray(vc.onCall) ? vc.onCall : []).map(toE164).filter(Boolean);
  const main = toE164(vc.transferTo || '');
  const chain = night ? [...onCall, main] : [main, ...onCall];
  return [...new Set(chain.filter(Boolean))];
}
/** 8pm-8am in the account's zone — when the owner is presumed asleep. */
function isNightFor(account, now = new Date()) {
  try {
    const h = Number(new Intl.DateTimeFormat('en-US', { timeZone: tzFor(account), hour: 'numeric', hour12: false }).format(now));
    return h < 8 || h >= 20;
  } catch { return false; }
}

/**
 * Normalise a transcript for speculation matching: Twilio's final result
 * usually differs from the last partial only in casing and punctuation, and
 * those differences must not throw a cooked answer away.
 */
...
async function warmCache(call) {
  try {
    ...
    await callAI('Reply with the single word: ready', { maxTokens: 3, tier: 'voice', timeoutMs: 6000, system });
...
async function think(call, heard) {
  ...
  // Last 16 turns only: the uncached part of the prompt grows with the call,
  // and a long call's history adds real think latency. Sixteen turns is more
  // context than any booking needs.
  const history = call.turns.slice(-16).map((t) => `${t.who === 'caller' ? 'Caller' : 'You'}: ${t.text}`).join('\n');
  ...
  const t0 = Date.now();
  // 9 seconds, hard. Twilio abandons a webhook at 15; TTS synthesis and the
  // round-trips eat several of those. Better to hit the catch below and say
  // "give me a second" than to let Twilio say "an application error has
  // occurred" for us.
  // 220, up from 110. The demo prompt ORDERS a one-breath capability answer
  // that is ~120 tokens with the JSON wrapper; capping at 110 truncated
  // exactly the replies that mattered most, and the parse failure turned them
  // into the fallback line. Latency cost is only the tokens actually used.
  const raw = await callAI(prompt, { maxTokens: 220, tier: 'voice', timeoutMs: 9000, system });
  ...
  return {
    ...
    // that" — the caller DID say something. And never a contentless murmur
    // either: "Go on — I'm with you" after a direct question was the single
    // dumbest thing this agent ever said on a live demo. Own the stumble in
    // one honest beat and re-invite the question; salvage above makes this
    // fallback genuinely rare.
    say: text || "Sorry, I tripped over myself there — ask me that once more?",
    action: ['continue', 'book', 'transfer', 'dnc', 'end', 'spam'].includes(j.action) ? j.action : 'continue',
...
module.exports = {
  ...
  salvageFromTranscript, isNoise, normSpeech, transferChain, isNightFor,
