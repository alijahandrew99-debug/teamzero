...
const SPEECH_TIMEOUT = '2';         // seconds of silence before we stop listening.
                                    // "auto" can hold the recogniser open far longer.
const SAY_VOICE = process.env.VOICE_TTS || 'Polly.Joanna-Neural';
...
function estimateCost({ durationSec = 0, turns = 0 }) {
  const mins = Math.max(1, Math.ceil(durationSec / 60));
  const gathers = Math.max(1, turns);
  return Number((mins * RATE_VOICE_MIN + gathers * RATE_RECOGNITION + gathers * RATE_AI_TURN).toFixed(3));
}
...
function sayAndGather(text, action) {
  ...
  return twiml(
    `<Gather input="speech" action="${esc(action)}" method="POST"`
    + ` speechTimeout="${SPEECH_TIMEOUT}" speechModel="phone_call" enhanced="true"`
...
function systemPrompt(profile, account, { direction, lead }) {
  ...
  return [
    ...
    '',
    `=== HOW TO SPEAK ===`,
    `- Two to three sentences per turn. Long enough to move things forward, short enough to stay a conversation.`,
    `- Be EFFICIENT: every back-and-forth costs the business money, so don't drip-feed. If you need three facts, ask for them in one natural sentence ("what's your name and the best number for you?") rather than three separate turns.`,
    `- Never re-ask something they already told you. Never summarise back what they just said.`,
