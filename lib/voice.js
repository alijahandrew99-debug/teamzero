 ...
 */
function sayAndGather(text, action, voiceName, extraHints) {
  ...
  return twiml(
    ...
    + ` bargeIn="true" maxSpeechTime="30" hints="${esc(hints)}"`
...
async function salvageFromTranscript(call) {
  ...
  const prompt = [
    ...
    ``,
    `Today is ${new Date().toISOString().slice(0, 10)}. Their number as dialled is ${call.direction === 'inbound' ? (call.from || 'unknown') : (call.to || 'unknown')}.`,
    ``,
    transcript,
    ``,
  ...
  try {
    ...
    return {
      ...
      whenISO: normaliseWhen(j.whenISO),
