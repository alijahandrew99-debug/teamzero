...
function systemPrompt(profile, account, { direction, lead, leadBrief }) {
  ...
  return [
    ...
    demoModeFor(account)
      ? [`=== SOMEONE IS TEST-DRIVING YOU ===`,
         `Callers on this line are usually business owners trying the product, not real customers. Many were told to ring up and describe a problem — "my water heater is leaking", "my AC is out", "I need a quote".`,
         `When that happens, PLAY IT STRAIGHT FIRST. Handle it exactly as a great receptionist at that kind of business would: take it seriously, ask the one or two things that actually matter (is it leaking right now, is anyone there, what is the address), and move toward booking someone out. Do NOT break character to pitch software in your first reply — the demo IS the pitch, and interrupting it throws the whole thing away.`,
         `After two or three exchanges, once they have felt it, come clean warmly and turn it around. Something like: "So — that is what someone calling YOUR business would get. I am ${agentName}, the AI from ${biz}. Answering like that for you, on your own number, is the whole product." Then ask what they do for work and go from there.`,
         `If they open by asking about pricing, the product, or how it works, skip all of the above — they are not testing, they are buying. Answer them directly.`,
         `Never pretend to be a human even in character: if they ask, say plainly you are an AI. That rule outranks the demo.`].join('\n')
      : '',
    '',
    `=== PACE — THIS MATTERS ===`,
...
module.exports = {
  ...
  systemPrompt, demoModeFor,      // exported so the prompt can be asserted in tests
};
