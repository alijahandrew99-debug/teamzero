...
// [] means "looked, found nothing" (NODATA/NXDOMAIN). null means "couldn't
// look" (timeout, SERVFAIL, network). Collapsing the two made a resolver hiccup
// look identical to a missing SPF record — and then blocked all sending for ten
// minutes on a cached false verdict.
async function txt(name) {
  try {
    ...
    const code = e && e.code;
    if (code === 'ENOTFOUND' || code === 'ENODATA') return [];   // genuinely absent
    return null;                                                  // resolver problem
  }
}

async function checkSPF(domain) {
  const raw = await txt(domain);
  if (raw === null) return { ok: false, status: 'lookup-failed', detail: "Couldn't reach DNS to check SPF — this is a lookup problem, not a missing record." };
  const recs = raw.filter((r) => /^v=spf1\b/i.test(r));
  if (!recs.length) {
...
async function checkDMARC(domain) {
  const raw = await txt(`_dmarc.${domain}`);
  if (raw === null) return { ok: false, status: 'lookup-failed', detail: "Couldn't reach DNS to check DMARC — lookup problem, not a missing record." };
  const recs = raw.filter((r) => /^v=DMARC1\b/i.test(r));
  if (!recs.length) {
...
async function checkDKIM(domain, extraSelector) {
  ...
  for (const sel of sels) {
    ...
    if (recs === null) { anyLookupFailed = true; continue; }
  ...
  if (anyLookupFailed) return { ok: false, status: 'lookup-failed', detail: "Couldn't reach DNS to check DKIM — lookup problem, not a missing key." };
  return { ok: false, status: 'missing',
    detail: 'No DKIM key found on any common selector. Turn on DKIM signing with your email provider, then re-check. If you use a custom selector, enter it above.' };
}
...
async function checkDomain(email, { selector = '', force = false } = {}) {
  ...
  const lookupFailed = [spf, dkim, dmarc].some((r) => r.status === 'lookup-failed');
  const val = {
  ...
  // A DNS outage is not evidence of a misconfigured domain — don't cache it.
  if (!lookupFailed) cache.set(key, { at: Date.now(), val });
  return val;
}
