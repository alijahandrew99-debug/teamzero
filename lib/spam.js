...
/** The stored row for this number, decayed or not. */
function stored(accountId, number) {
  const k = key(accountId, number);
  if (!k) return null;
  return db.readCollection('spamwatch').find((x) => x.id === k) || null;
}

function rec(accountId, number) {
  const r = stored(accountId, number);
  if (!r) return null;
...
function strike(accountId, number, points, why) {
  const k = key(accountId, number);
  if (!k) return 0;
  // Points carry over only from a LIVE record — a decayed one is a clean
  // slate, exactly as strikes() reports it. Whether we patch or insert is a
  // separate question, and it turns on whether a row physically exists:
  // deciding it from the decayed view inserted a SECOND row under the same id,
  // and every later read found the stale one first and reported 0 points. A
  // number that went quiet for 30 days could then never be struck again, so
  // the auto-block stopped working for exactly the repeat robocallers it is
  // for, and spamwatch grew a duplicate row per strike forever.
  const existing = rec(accountId, number);
  const row = stored(accountId, number);
  const total = (existing ? existing.points : 0) + points;
  if (row) db.patchCollection('spamwatch', k, { points: total, at: new Date().toISOString(), why });
