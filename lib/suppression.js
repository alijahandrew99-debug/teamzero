...
/**
 * Phone numbers live in the same suppression list as email addresses, keyed as
 * <digits>@phone.invalid. One list, one opt-out: someone who says "stop calling"
 * on a call is also never emailed, and vice versa.
 */
function phoneKey(number) {
  let d = String(number || '').replace(/[^0-9]/g, '');
  if (!d) return '';
  // Normalise to a single canonical form, or "(213) 682-2616" and
  // "+1 213 682 2616" become two different keys and a do-not-call recorded on
  // an inbound call would not block the outbound redial.
  if (d.length === 10) d = '1' + d;                       // US local -> +1
  if (d.length === 11 && d.startsWith('1')) return `${d}@phone.invalid`;
  return `${d}@phone.invalid`;
}
function isPhoneSuppressed(accountId, number) {
  const k = phoneKey(number);
  if (!k) return { blocked: false };
  return isSuppressed(accountId, k);
}
function suppressPhone(accountId, number, reason = 'unsubscribe', opts = {}) {
  const k = phoneKey(number);
  return k ? suppress(accountId, k, reason, opts) : null;
}

module.exports = { isSuppressed, suppress, unsuppress, phoneKey, isPhoneSuppressed, suppressPhone, tokenFor, parseToken, isHardBounce, REASONS, norm, domainOf };
