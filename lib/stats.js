...
/**
 * What we know about a caller from their NUMBER: past bookings and past calls
 * with this business. Recognising a repeat caller is the difference between
 * an AI that feels like staff ("Hi Maria — same stylist as last time?") and one
 * that feels like an IVR. Only Dialzara and the FSM bundles do this; it is
 * where Dawnpipe pulls ahead rather than catches up.
 *
 * Returns null for a first-time caller. Never invents: only facts on file.
 */
function callerHistory(accountId, fromNumber, phoneKeyFn) {
  const key = phoneKeyFn(fromNumber || '');
  if (!key) return null;
  const same = (n) => phoneKeyFn(n || '') === key;
  const appts = db.getAppointments(accountId).filter((a) => same(a.phone)).sort((x, y) => String(y.createdAt).localeCompare(String(x.createdAt)));
  const calls = db.getCalls(accountId, 3000).filter((c) => same(c.from) && (c.transcript || []).length).sort((x, y) => String(y.at).localeCompare(String(x.at)));
  if (!appts.length && !calls.length) return null;
  const name = (appts.find((a) => a.name && a.name !== 'Caller') || {}).name || '';
  const last = appts[0];
  const lines = [];
  if (name) lines.push(`Name on file: ${name}`);
  lines.push(`Has called ${calls.length} time(s) before; ${appts.length} previous booking(s).`);
  if (last) {
    const when = last.startsAt ? new Date(last.startsAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : (last.whenText || 'unscheduled');
    lines.push(`Most recent: ${when}${last.reason ? ' — ' + last.reason : ''} (${last.status || 'booked'})`);
  }
  const reasons = [...new Set(appts.map((a) => (a.reason || '').trim()).filter(Boolean))].slice(0, 3);
  if (reasons.length) lines.push(`Past reasons: ${reasons.join('; ')}`);
  return { name, count: calls.length, bookings: appts.length, last, brief: lines.join('\n') };
}

// Once-per-number-per-day guard for missed-call text-back. In memory: a
// restart forgetting it just means one extra text, which is a small price
// against the alternative of persisting yet another table.
const _texted = new Map();
function _dayKey(accountId, phone) { return `${accountId}|${phone}|${new Date().toISOString().slice(0, 10)}`; }
function textedToday(accountId, phone) { return _texted.has(_dayKey(accountId, phone)); }
function markTexted(accountId, phone) {
  _texted.set(_dayKey(accountId, phone), Date.now());
  if (_texted.size > 5000) for (const k of [..._texted.keys()].slice(0, 1000)) _texted.delete(k);
}

module.exports = { forRange, summary, recapEmail, callerHistory, textedToday, markTexted };
